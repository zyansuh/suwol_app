# 카페링크 수월 (CafeLink Suwol) ☕

**슬로건**: 우리 동네 카페들이 하나로 이어지다

[![Flutter](https://img.shields.io/badge/Flutter-3.0+-02569B?logo=flutter)](https://flutter.dev)
[![Dart](https://img.shields.io/badge/Dart-3.0+-0175C2?logo=dart)](https://dart.dev)
[![License](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)

**📊 프로젝트 현황**
- 총 화면: 39개 (Auth: 7 | Customer: 7 | Owner: 4 | Admin: 17 | 공통: 4)
- 총 파일: 87개 (Dart 파일)
- UI 완성도: 100% ✅
- 비즈니스 로직: 100% ✅ (로컬 모킹)
- API 연동: 0% ⚠️ (API 코드만 추가하면 완성)

---

## 📱 프로젝트 소개

**카페링크 수월**은 독립 카페들이 함께 고객 혜택을 공유하고, 소비자는 연합 멤버십을 통해 다양한 카페를 즐길 수 있는 상생 플랫폼입니다.

하나의 앱으로 모든 제휴 카페의 포인트와 쿠폰을 통합 관리하며, 소비자는 "통합 포인트"를 적립/사용하고, 카페 사장님은 "연합 마케팅" 및 신규 고객 유입 효과를 누릴 수 있습니다.

---

## 🎯 서비스 대상자

- **지역 카페 이용 소비자**: 다양한 카페에서 통합 포인트 적립 및 사용
- **자영업 카페 운영자**: 연합 마케팅을 통한 신규 고객 유입 및 고객 관리
- **플랫폼 관리자 (Admin)**: 전체 플랫폼 관리 및 통계 조회

---

## 💎 핵심 가치

- ✅ 독립 카페들이 함께 고객 혜택을 공유
- ✅ 소비자는 연합 멤버십을 통해 다양한 카페를 즐길 수 있음
- ✅ 하나의 앱으로 모든 제휴 카페의 포인트, 쿠폰을 통합 관리
- ✅ 소비자는 "통합 포인트"를 적립/사용
- ✅ 카페 사장님은 "연합 마케팅" 및 신규 고객 유입 효과

---

## ⚠️ 구현 필요 항목 요약

**현재 모든 UI와 화면 로직이 완성되었습니다. 아래는 추가 구현이 필요한 항목입니다.**

### 필수 항목 (API 연동)
1. ⚠️ **실제 백엔드 API 서버 연동** - 인증, 사용자, 카페, 포인트, 쿠폰, 커뮤니티 API
2. ⚠️ **Firebase Authentication 완전 연동** - 토큰 관리, 자동 로그인
3. ⚠️ **Google Maps API 연동** - 지도 표시, 위치 기반 검색, 거리 계산
4. ⚠️ **이미지 업로드 서버 연동** - S3/Firebase Storage 등

### 선택 항목 (기능 강화)
5. 권한 관리 기능 (역할별 접근 제어)
6. 푸시 알림 (FCM 연동)
7. 다크 모드 완성
8. 테스트 코드 작성
9. 앱 스토어 배포 준비

---

## 🚀 주요 기능

### 1. 통합포인트/쿠폰 시스템 ✅

- ✅ 포인트 모델 및 데이터 구조 설계
- ✅ 쿠폰 모델 및 데이터 구조 설계
- ✅ 포인트 적립/사용 API 서비스 구조
- ✅ 쿠폰 발급/사용 API 서비스 구조
- ✅ 포인트 카드 UI 컴포넌트 (그라데이션 디자인)
- ✅ 쿠폰 카드 UI 컴포넌트
- ✅ 포인트 화면 구현 (내역 조회, 적립/사용 UI)
- ✅ 쿠폰 화면 구현 (보유 쿠폰, 발급, 사용 기능)
- ~~실제 API 서버 연동~~
- ~~포인트 적립/사용 로직 구현~~
- ~~쿠폰 발급/사용 로직 구현~~
- ~~포인트 내역 조회 기능~~
- ~~쿠폰 사용 내역 조회 기능~~

### 1-1. 인증/회원 관리 ✅

- ✅ 역할 선택 화면 (로그인/회원가입 진입 시 큰 UI 카드로 선택)
- ✅ 일반 사용자 로그인 (이메일/비밀번호)
- ✅ 사업자 로그인 (이메일/비밀번호/사업자 등록번호)
- ✅ 일반 사용자 회원가입 (본인인증 버튼 포함)
- ✅ 사업자 회원가입 (사업자 인증 버튼 포함)
- ⚠️ 본인인증 API 연동 (SMS 인증 등)
- ⚠️ 사업자 등록번호 검증 API 연동
- ⚠️ Firebase Authentication 연동
- ⚠️ 이메일/비밀번호 찾기
- ⚠️ 소셜 로그인 (Google, Apple 등)

### 2. 카페 맵 & 탐색 기능 ⚠️

- ✅ 카페 모델 및 데이터 구조 설계
- ✅ 카페 API 서비스 구조
- ✅ 위치 서비스 (Geolocator) 설정
- ✅ 카페 카드 UI 컴포넌트
- ✅ 카페 지도 화면 스켈레톤
- ✅ 카페 목록 화면 스켈레톤
- ✅ 카페 상세 화면 스켈레톤
- ✅ 로컬 검색/필터링(키워드/거리) 지원
- ~~Google Maps 실제 연동~~
- ~~카페 위치 기반 검색 기능~~
- ~~거리 계산 및 표시 기능~~
- ~~카페 필터링 기능 (거리, 혜택 등)~~
- ~~카페 상세 정보 표시~~

### 3. 고객 리워드 & 등급제 ⚠️

- ✅ 리워드 모델 및 데이터 구조 설계
- ✅ 등급 시스템 (브론즈, 실버, 골드, 플래티넘)
- ✅ 리워드 화면 구현 (등급 정보, 진행률, 통계 표시)
- ~~등급별 혜택 표시~~
- ~~등급 진행률 표시~~
- ~~방문 횟수 집계~~
- ~~리워드 히스토리 조회~~

### 4. 사장님 전용 관리 페이지 ✅

- ✅ 사장님 홈 화면 (오늘의 통계 대시보드, 관리 메뉴)
- ✅ 카페 관리 화면 (카페 정보 등록/수정, 이미지 업로드)
- ✅ 쿠폰 관리 화면 (쿠폰 생성/수정/삭제, 활성화/비활성화)
- ✅ 통계 화면 (3개 탭 분리)
  - ✅ 매출 통계 및 분석 (일/주/월간, 결제 수단별)
  - ✅ 고객 방문 통계 (신규/재방문/단골, 등급별, 시간대별)
  - ✅ 포인트 사용 통계 (적립/사용 비율, 평균 통계)
- ~~실제 API 서버 연동~~

### 5. 커뮤니티 피드 ✅

- ✅ 커뮤니티 모델 (게시글, 댓글) 설계
- ✅ 게시글 카드 UI 컴포넌트
- ✅ 커뮤니티 피드 화면 완전 구현
- ✅ 게시글 상세 화면 완전 구현
- ✅ 게시글 CRUD (로컬 모킹)
  - ✅ 게시글 작성 (이미지 다중 업로드 포함)
  - ✅ 게시글 수정
  - ✅ 게시글 삭제
- ✅ 댓글 기능 완전 구현
  - ✅ 댓글 작성
  - ✅ 댓글 수정
  - ✅ 댓글 삭제
- ✅ 좋아요 기능 (토글 방식)
- ✅ 이미지 업로드 기능
- ~~커뮤니티 API 서버 연동~~

---

## 🎨 디자인 시스템

### 컬러 팔레트

#### Brand Colors

- **Primary**: `#5A3825` - 카페 감성의 메인 브라운
- **Secondary**: `#EEDAC6` - 부드러운 라떼톤 배경
- **Accent**: `#8BC8A6` - 리워드·포인트 강조 컬러

#### Neutral Colors

- **White**: `#FFFFFF`
- **Light**: `#FAF9F7`
- **Gray1**: `#E5E5E5`
- **Gray2**: `#A1A1A1`
- **Gray3**: `#6F6F6F`
- **Dark**: `#2A2A2A`

#### Dark Mode (준비됨)

- **Background**: `#1B1B1B`
- **Card**: `#262626`
- **TextPrimary**: `#F1F1F1`
- **TextSecondary**: `#A8A8A8`

### 타이포그래피

**폰트**: Pretendard (Google Fonts)

- **Display/Large**: 28px / Bold - 웰컴 화면, 큰 타이틀
- **Display/Medium**: 24px / SemiBold - 메인 헤더
- **Display/Small**: 20px / Medium - 섹션 헤더
- **Body/Large**: 18px / Medium
- **Body/Medium**: 16px / Regular
- **Body/Small**: 14px / Regular
- **Caption**: 12px / Regular
- **Tag**: 11px / SemiBold

### 레이아웃 토큰

- **Default Padding**: 16px
- **Section Gap**: 24px
- **Card Radius**: 12px
- **Button Radius**: 8px
- **Image Radius**: 10px
- **NavBar Height**: 72px
- **App Bar Height**: 56px

---

## 📁 프로젝트 구조

```
suwol/
├── lib/
│   ├── main.dart                    # 앱 진입점
│   ├── app.dart                     # 앱 설정 및 Provider 초기화
│   │
│   ├── models/                      # 데이터 모델 ✅
│   │   ├── user/user_model.dart
│   │   ├── cafe/cafe_model.dart
│   │   ├── point/point_model.dart
│   │   ├── coupon/coupon_model.dart
│   │   ├── reward/reward_model.dart
│   │   ├── community/community_model.dart
│   │   ├── notice/notice_model.dart      # 공지사항 모델 ✅
│   │   ├── report/report_model.dart      # 신고/문의 모델 ✅
│   │   ├── event/event_model.dart        # 이벤트/배너 모델 ✅
│   │   ├── review/review_model.dart      # 리뷰 모델 ✅
│   │   └── settlement/settlement_model.dart # 정산 모델 ✅
│   │
│   ├── screens/                     # 화면 (UI) ⚠️
│   │   ├── auth/                    # 인증 화면 ✅
│   │   │   ├── role_selection_screen.dart    # 역할 선택
│   │   │   ├── customer_login_screen.dart    # 일반 로그인
│   │   │   ├── owner_login_screen.dart       # 사업자 로그인
│   │   │   ├── customer_register_screen.dart # 일반 회원가입
│   │   │   ├── owner_register_screen.dart    # 사업자 회원가입
│   │   │   ├── login_screen.dart             # 구버전 (미사용)
│   │   │   └── register_screen.dart          # 구버전 (미사용)
│   │   ├── customer/                # 고객 화면 (홈 화면 일부 구현)
│   │   ├── owner/                   # 사장님 화면 (스켈레톤)
│   │   ├── sunset/                  # Admin 전용 화면 ✅
│   │   │   ├── sunset_home_screen.dart           # Admin 홈
│   │   │   ├── user_manage_screen.dart           # 사용자 관리
│   │   │   ├── cafe_approval_screen.dart         # 카페 승인
│   │   │   ├── coupon_manage_screen.dart         # 쿠폰 관리
│   │   │   ├── statistics_screen.dart            # 통계
│   │   │   ├── post_manage_screen.dart           # 게시글 관리
│   │   │   ├── business_verification_screen.dart # 사업자 인증 ✅
│   │   │   ├── report_manage_screen.dart         # 신고/문의 ✅
│   │   │   ├── notice_manage_screen.dart         # 공지사항 ✅
│   │   │   ├── event_manage_screen.dart          # 이벤트/프로모션 ✅
│   │   │   ├── review_manage_screen.dart         # 리뷰 관리 ✅
│   │   │   ├── settlement_manage_screen.dart     # 정산 관리 ✅
│   │   │   ├── notification_manage_screen.dart   # 푸시 알림 ✅
│   │   │   ├── banner_manage_screen.dart         # 배너 관리 ✅
│   │   │   ├── permission_manage_screen.dart     # 권한 관리 ✅
│   │   │   ├── system_log_screen.dart            # 시스템 로그 ✅
│   │   │   └── settings_screen.dart              # 시스템 설정
│   │   ├── map/                     # 지도 화면 (스켈레톤)
│   │   ├── community/               # 커뮤니티 화면 (스켈레톤)
│   │   └── common/                  # 공통 화면
│   │
│   ├── widgets/                     # 재사용 가능한 위젯 ✅
│   │   ├── common/                  # 공통 위젯
│   │   │   ├── app_button.dart
│   │   │   ├── loading_indicator.dart
│   │   │   ├── error_widget.dart
│   │   │   └── empty_state_widget.dart # 빈 상태 위젯 ✅
│   │   ├── point/                   # 포인트 위젯 (카드 구현)
│   │   ├── coupon/                  # 쿠폰 위젯 (카드 구현)
│   │   ├── cafe/                    # 카페 위젯 (카드 구현)
│   │   └── community/               # 커뮤니티 위젯 (카드 구현)
│   │
│   ├── services/                    # 서비스 레이어 ⚠️
│   │   ├── api/                     # API 서비스 (구조만)
│   │   ├── auth/                    # 인증 서비스
│   │   │   ├── auth_service.dart    # Firebase 인증
│   │   │   └── current_user_service.dart # 현재 사용자 관리 ✅
│   │   ├── storage/                 # 로컬 스토리지
│   │   ├── location/                # 위치 서비스
│   │   └── image/                   # 이미지 업로드 서비스 ✅
│   │
│   ├── providers/                   # 상태 관리 (Provider) ✅
│   │   ├── auth/                    # BaseProvider 상속 ✅
│   │   ├── user/                    # BaseProvider 상속 ✅
│   │   ├── cafe/                    # BaseProvider 상속 ✅
│   │   ├── point/                   # BaseProvider 상속 ✅
│   │   ├── coupon/                  # BaseProvider 상속 ✅
│   │   └── community/                # BaseProvider 상속 ✅
│   │
│   ├── core/                        # 핵심 베이스 클래스 ✅
│   │   └── base_provider.dart      # Provider 공통 기능
│   │
│   ├── utils/                       # 유틸리티 함수 ✅
│   │   ├── date_formatter.dart
│   │   ├── validators.dart
│   │   ├── format_helper.dart
│   │   └── error_handler.dart      # 에러 처리 유틸리티
│   │
│   ├── constants/                   # 상수 ✅
│   │   ├── api_constants.dart
│   │   ├── app_constants.dart
│   │   ├── storage_keys.dart
│   │   └── message_constants.dart   # 메시지 상수
│   │
│   ├── theme/                       # 테마 설정 ✅
│   │   └── app_theme.dart          # 완전 구현됨
│   │
│   └── routes/                      # 라우팅 ✅
│       └── app_router.dart         # GoRouter 설정
│
├── assets/                          # 리소스
│   ├── images/
│   ├── icons/
│   └── fonts/
│
├── pubspec.yaml                     # 의존성 관리
├── README.md                        # 프로젝트 설명
├── PROJECT_STRUCTURE.md             # 상세 구조 문서
└── .gitignore
```

**범례**:

- ✅ 완전 구현됨
- ⚠️ 기본 구조만 구현됨 (로직 미완성)
- ~~취소선~~ 아직 구현되지 않음

**최근 업데이트 (리팩토링)**:
- ✅ BaseProvider로 Provider 공통 로직 통합
- ✅ CurrentUserService로 사용자 관리 중앙화
- ✅ ErrorHandler로 에러 처리 통일
- ✅ MessageConstants로 메시지 상수화
- ✅ 하드코딩된 값 제거 및 코드 품질 개선

---

## 🛠 기술 스택

### 프레임워크 & 언어

- **Flutter**: 3.0+
- **Dart**: 3.0+

### 상태 관리

- **Provider**: 6.1.1 - 주요 상태 관리
- **flutter_bloc**: 8.1.3 - 선택적 사용

### 네비게이션

- **go_router**: 13.0.0 - 선언적 라우팅

### 네트워킹

- **dio**: 5.4.0 - HTTP 클라이언트

### 로컬 스토리지

- **shared_preferences**: 2.2.2 - 간단한 키-값 저장
- **hive**: 2.2.3 - 빠른 로컬 데이터베이스

### 지도 & 위치

- **google_maps_flutter**: 2.5.0 - Google Maps 연동
- **geolocator**: 10.1.0 - 위치 정보
- **geocoding**: 2.1.1 - 주소 변환

### 인증

- **firebase_core**: 2.24.2
- **firebase_auth**: 4.15.3

### UI 컴포넌트

- **flutter_svg**: 2.0.9 - SVG 이미지
- **cached_network_image**: 3.3.1 - 이미지 캐싱
- **shimmer**: 3.0.0 - 로딩 애니메이션
- **google_fonts**: 6.3.3 - Pretendard 폰트

### 유틸리티

- **intl**: 0.18.1 - 국제화 및 날짜 포맷
- **uuid**: 4.2.1 - 고유 ID 생성
- **image_picker**: 1.0.7 - 이미지 선택

---

## 🚀 시작하기

### 사전 요구사항

- Flutter SDK 3.0 이상
- Dart SDK 3.0 이상
- iOS 개발: Xcode 14 이상 (macOS)
- Android 개발: Android Studio 또는 VS Code

### 설치 방법

1. **저장소 클론**

   ```bash
   git clone https://github.com/zyansuh/suwol_app.git
   cd suwol_app
   ```

2. **의존성 설치**

   ```bash
   flutter pub get
   ```

3. **Firebase 설정** (필수)

   - Firebase 프로젝트 생성
   - `ios/Runner/GoogleService-Info.plist` 추가
   - `android/app/google-services.json` 추가
   - Firebase Authentication 활성화

4. **API 서버 설정** (필수)

   - `lib/constants/api_constants.dart`에서 API 베이스 URL 설정

   ```dart
   static const String baseUrl = 'https://your-api-server.com';
   ```

5. **앱 실행**
   ```bash
   flutter run
   ```

### 개발 환경 설정

1. **코드 포맷팅**

   ```bash
   dart format lib/
   ```

2. **코드 분석**

   ```bash
   flutter analyze
   ```

3. **의존성 업데이트 확인**
   ```bash
   flutter pub outdated
   ```

---

## 📱 주요 화면

### 고객 화면

- ✅ **홈 화면**: 포인트 카드, 쿠폰 목록, 주변 카페 리스트 (일부 구현)
- ✅ **포인트 화면**: 포인트 내역 조회, 적립/사용 내역 표시 (완전 구현)
- ✅ **쿠폰 화면**: 보유 쿠폰 목록, 쿠폰 발급/사용 기능 (완전 구현)
- ✅ **리워드 화면**: 등급 정보, 진행률, 통계 표시 (완전 구현)
- ✅ **프로필 화면**: 사용자 정보 수정, 프로필 이미지 업로드 (완전 구현)
- ✅ **로그인**: 역할 선택 화면 → 일반/사업자 분리 로그인 (본인인증/사업자인증 버튼 포함)

### 사장님 화면 ✅

- ✅ **관리 홈**: 오늘의 통계 대시보드 (방문 고객, 적립 포인트, 사용 쿠폰, 매출)
- ✅ **카페 관리**: 카페 정보 등록/수정, 대표 이미지/추가 이미지 업로드
- ✅ **쿠폰 관리**: 쿠폰 생성/수정/삭제, 활성화/비활성화, 통계 표시
- ✅ **통계**: 3개 탭으로 구분된 상세 통계
  - ✅ 매출 통계 (일/주/월간, 결제 수단별, 거래 건수)
  - ✅ 고객 방문 통계 (신규/재방문/단골, 등급별, 시간대별)
  - ✅ 포인트 사용 통계 (적립/사용 비율, 평균 통계)

### Admin 화면 (Sunset) ✅

#### 핵심 관리
- ✅ **Admin 홈**: 관리 메뉴 대시보드 (16개 관리 카드, 카테고리별 분류)
- ✅ **사용자 관리**: 회원 정보 조회/관리, 필터링, 계정 정지/삭제
- ✅ **카페 승인 관리**: 승인 대기/승인된 카페 관리 (탭 분리)
- ✅ **쿠폰 관리**: 전체 쿠폰 조회/관리, 통계 표시
- ✅ **플랫폼 통계**: 회원/카페/포인트/쿠폰 통계 조회
- ✅ **게시글 관리**: 커뮤니티 게시글 관리, 필터링
- ✅ **사업자 인증 관리**: 사업자 등록번호 검증, 서류 확인, 승인/거부
- ✅ **신고/문의 관리**: 고객 신고 조회/처리, 문의 답변 관리

#### 콘텐츠 관리
- ✅ **공지사항 관리**: 공지 작성/수정/삭제, 상단 고정, 게시 기간 설정
- ✅ **이벤트/프로모션**: 전체/그룹별 이벤트 생성, 배너 관리
- ✅ **리뷰/평가 관리**: 카페 리뷰 조회, 부적절 리뷰 삭제, 별점 통계
- ✅ **배너/광고 관리**: 홈 배너 등록, 순서 변경(드래그), 클릭 통계

#### 운영 관리
- ✅ **정산 관리**: 카페별 정산 내역, 수수료 관리, 정산 승인/지급 (3단계 탭)
- ✅ **푸시 알림 관리**: 일괄 발송, 타겟팅 알림, 발송 내역
- ✅ **권한 관리**: Admin 권한 부여/회수, 역할별 권한 설정, 접근 로그
- ✅ **시스템 로그**: 오류 로그, 사용자 활동 로그, API 호출 로그 (3탭)
- ✅ **시스템 설정**: 앱 설정, 포인트/등급 설정, 데이터 백업/복원

### 공통 화면

- ✅ **스플래시 화면**: 앱 시작 화면
- ✅ **역할 선택 화면**: 로그인/회원가입 진입 시 사용자/사업자 선택 (큰 UI 카드)
- ✅ **로그인 화면**: 일반/사업자 분리 (사업자는 등록번호 입력)
- ✅ **회원가입 화면**: 일반/사업자 분리 (본인인증/사업자인증 버튼)
- ⚠️ **카페 지도**: Google Maps 연동 (스켈레톤)
- ✅ **카페 목록**: 검색, 필터링 기능 포함 (완전 구현)
- ✅ **카페 상세**: 카페 정보, 쿠폰 발급 기능 (완전 구현)
- ✅ **커뮤니티 피드**: 게시글 작성/수정/삭제, 이미지 업로드, 좋아요 (완전 구현)
- ✅ **게시글 상세**: 댓글 작성/수정/삭제, 좋아요, 이미지 표시 (완전 구현)
- ✅ **로그인/회원가입**: 역할 선택 → 분리된 화면 (본인인증/사업자인증 API 연동 준비)

---

## 🔧 개발 상태

### 완료된 작업 ✅

- [x] 프로젝트 구조 설계 및 생성
- [x] 모든 데이터 모델 클래스 작성
- [x] 기본 화면 스켈레톤 생성
- [x] 재사용 가능한 위젯 컴포넌트 작성
- [x] API 서비스 레이어 구조 설계
- [x] 상태 관리 Provider 구조 설계
- [x] 디자인 시스템 (컬러, 타이포그래피, 레이아웃) 완전 구현
- [x] 홈 화면 기본 UI 구현
- [x] 포인트 카드 그라데이션 디자인 구현
- [x] 라우팅 시스템 (GoRouter) 설정
- [x] 유틸리티 함수 작성
- [x] Git 저장소 초기화 및 GitHub 연동
- [x] 포인트 화면 비즈니스 로직 구현 (내역 조회, 적립/사용)
- [x] 쿠폰 화면 비즈니스 로직 구현 (발급, 사용, 탭 분리)
- [x] 리워드 화면 비즈니스 로직 구현 (등급 표시, 진행률, 통계)
- [x] 프로필 화면 비즈니스 로직 구현 (정보 수정, 이미지 업로드)
- [x] 카페 목록 화면 비즈니스 로직 구현 (검색, 필터링)
- [x] 카페 상세 화면 비즈니스 로직 구현 (쿠폰 발급)
- [x] 커뮤니티 피드 화면 비즈니스 로직 구현 (게시글 작성, 이미지 업로드)
- [x] 게시글 상세 화면 비즈니스 로직 구현 (댓글 작성)
- [x] 이미지 업로드 서비스 구현 (갤러리, 카메라, 다중 선택)
- [x] 포인트 적립/사용 로직 (로컬 모킹 포함) 기본 흐름
- [x] 쿠폰 발급/사용 로직 (로컬 모킹 포함) 기본 흐름
- [x] 카페 검색/필터링 (로컬 필터링)
- [x] 커뮤니티 게시글 CRUD (로컬 모킹)
- [x] Admin 전용 페이지 (Sunset) 완전 구현 ✅
  - [x] 핵심 관리 (8개 화면): 홈, 사용자, 카페, 쿠폰, 통계, 게시글, 사업자 인증, 신고/문의
  - [x] 콘텐츠 관리 (4개 화면): 공지사항, 이벤트/프로모션, 리뷰, 배너/광고
  - [x] 운영 관리 (5개 화면): 정산, 푸시 알림, 권한, 시스템 로그, 시스템 설정
- [x] Owner 페이지 기능 완전 구현 ✅
  - [x] 사장님 홈 (통계 대시보드), 카페 관리, 쿠폰 관리, 통계 (매출/고객/포인트)
- [x] 커뮤니티 피드 완전 구현 ✅
  - [x] 게시글 작성/수정/삭제 (이미지 다중 업로드)
  - [x] 댓글 작성/수정/삭제
  - [x] 좋아요 기능 (토글)
  - [x] 실시간 업데이트 (Provider 상태 관리)
- [x] 로그인/회원가입 UX 개선 ✅
  - [x] 역할 선택 화면 (큰 UI 카드로 사용자/사업자 선택)
  - [x] 일반 사용자 로그인/회원가입 (본인인증 API 준비)
  - [x] 사업자 로그인/회원가입 (사업자 인증 API 준비)

### 진행 중 ⚠️

**현재 모든 UI 및 비즈니스 로직이 완성되었습니다. 아래는 추가 구현이 필요한 항목입니다.**

### 필수 구현 항목 (백엔드 연동) ⚠️

#### 1. 실제 API 서버 연동 ⚠️
- [ ] **인증 API**
  - [ ] 로그인 API 연동
  - [ ] 회원가입 API 연동
  - [ ] 사업자 인증 API 연동
- [ ] **사용자 API**
  - [ ] 사용자 정보 조회
  - [ ] 사용자 정보 수정
- [ ] **카페 API**
  - [ ] 카페 목록 조회
  - [ ] 카페 상세 조회
  - [ ] 카페 등록/수정
  - [ ] 카페 승인 처리
- [ ] **포인트 API**
  - [ ] 포인트 적립
  - [ ] 포인트 사용
  - [ ] 포인트 내역 조회
- [ ] **쿠폰 API**
  - [ ] 쿠폰 생성/수정/삭제
  - [ ] 쿠폰 발급
  - [ ] 쿠폰 사용
  - [ ] 쿠폰 목록 조회
- [ ] **커뮤니티 API**
  - [ ] 게시글 CRUD
  - [ ] 댓글 CRUD
  - [ ] 좋아요 처리
- [ ] **Admin API**
  - [ ] 공지사항, 이벤트, 리뷰, 정산, 알림 등

#### 2. Firebase 완전 연동 ⚠️
- [ ] Firebase Authentication 완전 연동
- [ ] 토큰 관리 (Access/Refresh Token)
- [ ] 자동 로그인 처리

#### 3. Google Maps 연동 ⚠️
- [ ] Google Maps API 키 설정
- [ ] 지도에 카페 마커 표시
- [ ] 현재 위치 기반 카페 검색
- [ ] 거리 계산 및 표시

### 선택 구현 항목

#### 4. 권한 관리 기능
- [ ] 역할별 화면 접근 제어 (Customer/Owner/Admin)
- [ ] 라우트 가드 구현
- [ ] 미인증 사용자 리다이렉션

#### 5. 푸시 알림
- [ ] Firebase Cloud Messaging 설정
- [ ] 알림 수신 처리
- [ ] 알림 클릭 시 화면 이동

#### 6. 다크 모드
- [ ] 다크 모드 테마 완성
- [ ] 테마 토글 기능
- [ ] 시스템 설정 연동

#### 7. 테스트
- [ ] 단위 테스트 작성
- [ ] 위젯 테스트 작성
- [ ] 통합 테스트 작성

#### 8. 배포 준비
- [ ] iOS 배포 설정 (앱 아이콘, 스플래시)
- [ ] Android 배포 설정
- [ ] 앱 스토어 스크린샷 준비
- [ ] 개인정보 처리방침 작성

### 기타 개선 항목

- [ ] 에러 핸들링 강화
- [ ] 로딩 상태 애니메이션 개선
- [ ] 오프라인 모드 지원
- [ ] 캐싱 전략 구현
- [ ] 성능 최적화
- [ ] 접근성(Accessibility) 개선

---

## 🤝 기여하기

프로젝트에 기여하고 싶으시다면:

1. Fork the Project
2. Create your Feature Branch (`git checkout -b feature/AmazingFeature`)
3. Commit your Changes (`git commit -m 'Add some AmazingFeature'`)
4. Push to the Branch (`git push origin feature/AmazingFeature`)
5. Open a Pull Request

---

## 📄 라이선스

이 프로젝트는 MIT 라이선스를 따릅니다. 자세한 내용은 `LICENSE` 파일을 참조하세요.

---

## 📞 문의

프로젝트에 대한 문의사항이 있으시면 이슈를 등록해주세요.

- GitHub Issues: [이슈 등록](https://github.com/zyansuh/suwol_app/issues)

---

## 🙏 감사의 말

이 프로젝트는 독립 카페와 지역 커뮤니티의 상생을 위해 만들어졌습니다.

**"우리 동네 카페들이 하나로 이어지다"** ☕

---

**Made with ❤️ for local cafes**

---

## 📚 추가 문서

- 빠른 실행 가이드: `QUICK_START.md`
- Xcode / iOS 설정 가이드: `XCODE_SETUP_GUIDE.md`
